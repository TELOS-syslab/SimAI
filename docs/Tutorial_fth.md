...
# SimAI 是一个综合的大规模 AI 训练仿真工具包，提供了三种主要仿真场景：

1. **SimAI-Analytical**  # 一个分析性仿真工具，用于抽象底层网络通信细节。
   # 它采用简化的方法，通过 busbw（总线带宽）来估算集合/点对点通信的通信时间，
   # 从而实现快速场景验证。主要应用场景包括但不限于：

   * *性能分析*: 比较不同模型的完成时间（例如，研究专家数量对 MoE 模型训练性能的影响）

   * *框架级别并行参数优化*: 平衡和优化 TP/EP/PP 参数以分析端到端的时间效果

   * *扩展探索*: 研究不同扩展域中的并行参数性能以进行特定场景优化

   * *扩展带宽选择*: 针对各种 GPU 性能研究具有成本效益的带宽配置

> 💡 *目前支持手动 busbw.yaml 配置。基于并行场景的自动 busbw 推断将很快开源。请保持关注，并随时联系我们以获取更多信息。✨*
...
2. **SimAI-Simulation(NS-3)**  # 一个高保真、全栈仿真工具，理论上可以与任何纯网络模拟器集成。
   # 它提供 LLM 训练期间通信行为的细粒度再现。目前支持 NS-3 作为网络后端
   # （我们鼓励集成新的网络仿真工具）。主要研究领域包括：

   * *集合通信算法研究*: 针对非交换架构和其他新型网络拓扑设计和优化集合通信流量模式

   * *网络协议研究*: 针对不同架构评估和优化网络协议、拥塞控制算法、路由机制及其他底层网络技术

   * *新型网络架构设计*: 探索创新的网络架构

> 💡 我们强烈鼓励研究人员基于 SimAI-Simulation 做出创新扩展和突破性研究，适合顶级会议。
   # 加入我们的社区或通过电子邮件与我们联系 - 我们致力于为有前景的研究方向提供技术支持！✨
...
SimAI-物理(TODO)
有关每个组件的其他功能，请参考 SimCCL 和 ns-3-alibabacloud。

🛠️ 环境设置

在正常情况下，运行 SimAI 需要使用 AICB 工具生成一个 Workload 文件。为了创建一个精确的 Workload，您可能需要利用 AIOB 功能来确定各种计算内核的时间，这需要一个 GPU 环境。因此，我们建议直接在最新的 NGC 镜像 中执行 SimAI 全栈工具包。

💡 重要说明：SimAI-Simulation 编译需要移除 ninja（NGC 镜像中预装）。您可以使用以下命令移除它：
